# Erlang `distributed_logging` OTP app

This directory contains a runnable Erlang/OTP application generated from the Scribble specification in `examples/scribble/DistributedLogging.scr`.
...
```erlang
application:start(distributed_logging).
```

Stop it with:

```erlang
application:stop(distributed_logging).
```
