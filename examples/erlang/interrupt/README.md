# Erlang `interrupt` OTP app

This directory contains a runnable Erlang/OTP application generated from the Scribble specification in `examples/scribble/Interrupt.scr`.
...
```erlang
application:start(interrupt).
```

Stop it with:

```erlang
application:stop(interrupt).
```
...
Source: This paper (Interrupt example)
