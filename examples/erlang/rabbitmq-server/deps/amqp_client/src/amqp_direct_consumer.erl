...
-export([init/1, state1/3, state2/3, state3/3, state4/3, state5/3, state7/3, state9/3, terminate/3]).

-record(state_data, {
    consumer_pid = undefined,
    amqp_state = undefined
}).
...
init([ConsumerPid]) ->
    ...
    {ok, state1, #state_data{consumer_pid = ConsumerPid}}.

%% state1/3: Corresponds to handling the basic.consume message
% -spec state1(atom(), {basic_consume}, #state_data{}) -> {next_state, state2, #state_data{}}.
state1(_EventType, {basic_consume}, StateData) ->
    % Handle basic.consume
    ConsumerPid = StateData#state_data.consumer_pid,
    ConsumerPid ! {basic_consume, self()},
    {next_state, state2, StateData}.

%% state2/3: Corresponds to handling basic_consume_ok message
% -spec state2(atom(), {pid(), {basic_consume_ok}}, #state_data{}) -> {next_state, state3, #state_data{}}.
state2(_EventType, {Pid, {basic_consume_ok}}, StateData) ->
    Pid ! {basic_consume_ok, self()},
    {next_state, state3, StateData}.

%% state3/3: Handles basic_cancel or temp
% -spec state3(atom(), {pid(), {basic_cancel}}, #state_data{}) -> {next_state, state4, #state_data{}} | {stop, normal, #state_data{}}.
state3(_EventType, {Pid, {basic_cancel}}, StateData) ->
    Pid ! {basic_cancel, self()},
    {next_state, state4, StateData}.

%% state4/3: Handles delivery or cancel logic
% -spec state4(atom(), {pid(), {basic_deliver}}, #state_data{}) -> {next_state, state5, #state_data{}}.
state4(_EventType, {Pid, {basic_deliver, Msg}}, StateData) ->
    Pid ! {basic_deliver, Msg},
    {next_state, state5, StateData}.

%% state5/3: Handles bogus1 or stops normal
% -spec state5(atom(), {bogus1}, #state_data{}) -> {stop, normal, #state_data{}}.
state5(_EventType, {bogus1}, StateData) ->
    {stop, normal, StateData}.

%% state7/3: Handles bogus2
% -spec state7(atom(), {bogus2}, #state_data{}) -> {stop, normal, #state_data{}}.
state7(_EventType, {bogus2}, StateData) ->
    {stop, normal, StateData}.

%% state9/3: Handles cancel_ok
% -spec state9(atom(), {pid(), {basic_cancel_ok}}, #state_data{}) -> {stop, normal, #state_data{}}.
state9(_EventType, {Pid, {basic_cancel_ok}}, StateData) ->
    Pid ! {basic_cancel_ok, self()},
    {stop, normal, StateData}.

%%---------------------------------------------------------------------------
%% Termination callback
%%---------------------------------------------------------------------------
% -spec terminate(any(), any(), #state_data{}) -> any().
terminate(_Reason, _Event, _StateData) ->
    ok.