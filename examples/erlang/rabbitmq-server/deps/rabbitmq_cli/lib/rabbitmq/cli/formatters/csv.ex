...
# add the csv code path if available (ignore failure)
_ = Code.append_path(Path.join(["..", "csv", "ebin"]))

# Guard CSV.Encode implementations to avoid compile errors if CSV lib is unavailable
defmodule CSV.EncodeGuard do
  if Code.ensure_loaded?(CSV.Encode) do
    defguard define_impls() when true
  end
end
