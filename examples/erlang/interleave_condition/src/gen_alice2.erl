...
start_link(CallbackModule, Args) ->
  case code:ensure_loaded(CallbackModule) of
    {module, CallbackModule} ->
      gen_statem:start_link({local, CallbackModule}, gen_alice2, {CallbackModule, Args}, [{debug, [trace, {log_to_file, "alice2_debug.log"}]}]);
    {error, Reason} ->
      {error, Reason}
  end.
...
s5(internal, {postponing}, Data) ->
  CallbackModule = get(callback_module),
  CallbackModule:s5(internal, {postponing}, Data).
