...
object CodegenCLI {
  ...
  def main(argv: Array[String]): Unit = {
    ...
    selected.foreach { case (fullName, gtype) =>
      ...
      println(s"[Codegen] Generating for protocol $simpleName: roles=${targetRoles.mkString(", ")}, outDir=${outDir.getPath}")
      ...
      targetRoles.foreach { r =>
        ...
        RuntimeModuleGenerator.generate(simpleName, roleAtom, efsm, outDir)
        CallbackModuleGenerator.generateCallback(simpleName, roleAtom, efsm, outDir, rolesLower)
  ...
  private def usage(): Unit = {
    println(
      s"""
         |Usage:
         |  runMain com.github.rhu1.gt.codegen.CodegenCLI <path/to/file.scr> [-proto Name] (-all | -roles R1 R2 ...) [-out ./generated]
         |""".stripMargin)
  }
}
